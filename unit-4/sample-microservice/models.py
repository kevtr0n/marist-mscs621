...
######################################################################
    ...
    def __find_by(attribute, value):
        ...
        # return [data for data in Data.__data if data.category == category]
    ...
    def find_by_name(name):
        """ Query that finds Datas by their name """
    ...
    def find_by_availability(available=True):
        """ Query that finds Datas by their availability """
        return Data.__find_by('available', available)
...
######################################################################
    ...
    def init_db(redis=None):
        ...
        if 'VCAP_SERVICES' in os.environ:
            Data.logger.info("Using VCAP_SERVICES...")
            vcap_services = os.environ['VCAP_SERVICES']
            services = json.loads(vcap_services)
            creds = services['rediscloud'][0]['credentials']
            Data.logger.info("Conecting to Redis on host %s port %s",
                            creds['hostname'], creds['port'])
            Data.connect_to_redis(creds['hostname'], creds['port'], creds['password'])
        else:
            Data.logger.info("VCAP_SERVICES not found, checking localhost for Redis")
            Data.connect_to_redis(os.environ['REDIS_ADDR'], os.environ['REDIS_PORT'], None)
            if not Data.redis:
                Data.logger.info("No Redis on localhost, looking for redis host")
                Data.connect_to_redis('redis', 6379, None)
        if not Data.redis:
